...
\
```{r, echo=FALSE, fig.cap="x", out.width="100%", fig.ext="pdf"}
knitr::include_graphics("/Users/agustin/Desktop/Papers/Canary_Island_Project/spatial_modularity_in_the_canary_islands/Results/null_models.pdf")
```
...
```{r}
#modules_with_lat_lon <- read.csv("modules_with_lat_lon_islands_as_layers.csv") 
...
#modules_with_lat_lon_id %>% 
 # ggplot(aes(x=module, y= count ,fill= factor(layer_name)))+ geom_bar(stat= "identity")+ theme_classic()+
  #scale_x_continuous(breaks=seq(0,88,4))+ labs(y="Number of islands", x="Module number")+
  #guides(fill=guide_legend(title="Island"))

Prop_sp_module_island <- read.csv("Prop_sp_module_island.csv") 

##-- Distribution of modules in islands with relative proportions
...
Shuffling local structure broke distance decay pattern in structure ($R^2_{M_2}$ = 0.12, P = 0.12) and 
had a significant effect on the overall structure variation explained by distance (P < 0.001).
\
\
```{r}
...
### Species distance decay- Null model shuffling species between layers (M1)
\ 
\
The decay was more pronounced when shuffling plants ($R^2_{M_1^P}$ = 0.7, P < 0.001) but much less so when shuffling pollinators ($R^2_{M_1^A}$ = 0.61, P < 0.001) and both species ($R^2_{M_1^{AP}}$ = 0.17, P < 0.001). These indicate that mainly pollinators has a stronger effect in the distance decay.
\ 
\ 
...
turnover_shuf_and_empirical %>% 
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.title = element_text(size=14, color='black'),
        axis.line = element_blank()) + 
  stat_cor(aes(label = ..rr.label..), label.x = 400, label.y = c(0.34, 0.32, 0.30, 0.28))+
  stat_cor(aes(label = ..p.label..), label.x = 380)
```
\
\
\
...
### Modules distance decay- Null model shuffling species between layers (M1)
\
\ 
...
jaccard_similarity_layer_empirical_and_null_no_self_loop_km %>% 
  ...
  theme(axis.text.y=element_text(size=15))+ theme(legend.title = element_text(size = 13), legend.text = element_text(size = 13))+
  labs(x="Distance in Km", y="Jaccard Similarity")+
   stat_cor(aes(label = ..p.label..), label.x = 390)+
  stat_cor(aes(label = ..rr.label..), label.x = 390, label.y = c(0.67, 0.63, 0.59, 0.55)) +
  theme(panel.grid = element_blank(),
        panel.border = element_rect(color = "black",fill = NA,size = 1),
        panel.spacing = unit(0.5, "cm", data = NULL),
        axis.text = element_text(size=14, color='black'),
...
\
\
In particular, redistributing plants did not affect the amount of variation explained by distance (P = 0.952). In contrast, redistributing pollinators (P < 0.001), or both plants and pollinators (P < 0.001) had a significant effect and almost produced distance decay disappear.
\
\
```{r}
...
### Modules distance decay- Null model shuffling interactions within layers (M2)
\
...
### Modules distance decay- Null model shuffling interactions between layers (M3)
\
...
### Modules distance decay- Null model uniform value interlayerlinks (M4)
\
