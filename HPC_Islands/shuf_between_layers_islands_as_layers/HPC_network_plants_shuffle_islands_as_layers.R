...
LOG <- function(s, appnd = T) {
  write_lines(s, paste("logs_plants/", JOB_ID, "_", trial_num,'_log.txt', sep=''), append = appnd)
}
...
interlayer_edges_shuf_plants <- read.csv("./interlayer_edges_shuf_plants_islands_as_layers.csv")

interlayers_shuf_plants <- NULL

interlayer_edges_run <- interlayer_edges_shuf_plants %>% filter(trial_number == trial_num)
for (i in 1:nrow(interlayer_edges_run)){ #for every species
  ...
  for (j in 4:ncol(interlayer_edges_run)){ #for all locations
   ...
   # if (current_species %in% interlayers_shuf$node_from & current_location %in% interlayers_shuf$layer_to &
    ...
    interlayers_shuf_plants <- rbind(interlayers_shuf_plants, tibble(trial_number = trial_num, layer_from = current_location,
                                                            node_from = current_species, 
                                                            layer_to = as.character(current_location_to), 
                                                            node_to = current_species))
   # } 
...
interlayers_new_shuf_plants <- interlayers_shuf_plants %>% subset(layer_to != "NA")
interlayers_new_shuf_plants <- interlayers_new_shuf_plants %>% subset(layer_from != layer_to)

write.csv(interlayers_new_shuf_plants, paste("csvs_plants/", trial_num,'_interlayers_new_shuf_plants_islands_as_layers.csv', sep=''), row.names = FALSE)
