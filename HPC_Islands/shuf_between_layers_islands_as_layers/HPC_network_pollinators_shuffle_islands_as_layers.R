...
LOG <- function(s, appnd = T) {
  write_lines(s, paste("logs_pollinators/", JOB_ID, "_", trial_num,'_log.txt', sep=''), append = appnd)
}
...
interlayer_edges_shuf <- read.csv("./interlayer_edges_shuf_islands_as_layers.csv")
...
write.csv(interlayers_new_shuf, paste("csvs_pollinators/", trial_num,'_interlayers_new_shuf_islands_as_layers.csv', sep=''), row.names = FALSE)
