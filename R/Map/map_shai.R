...
Canary<- world_coordinates %>% 
  filter(long > -18.53 & long < -12.9, lat > 27.2 & lat < 29.5)#Filter Canary Island region
...
Canary_map<-ggplot() +
  ...
  geom_segment(aes(x= x, xend= xend, y= y, yend= yend, color = turnover),
                              data= jaccard_similarity_on_map)+
  ...
  xlab("Longitude") +xlim(-18.53, -13.5)+ ylab("Latitude")+ ylim(27.2,29.4)+
