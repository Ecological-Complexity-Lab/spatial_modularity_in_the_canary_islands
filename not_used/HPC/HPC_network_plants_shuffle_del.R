...
LOG <- function(s, appnd = T) {
  write_lines(s, paste("logs_plants_del/", JOB_ID, "_", trial_num,'_log.txt', sep=''), append = appnd)
}
...
interlayer_edges_shuf <- read.csv("./interlayer_edges_plants_shuf_del.csv")
...
write.csv(interlayers_new_shuf, paste("csvs_plants_del/", trial_num,'_interlayers_new_shuf.csv', sep=''), row.names = FALSE)
